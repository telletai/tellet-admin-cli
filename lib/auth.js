  ...
  return async (options) => {
    ...
    try {
      ...
      if (!options.skipCache && !options.noCache) {
        const cached = await authManager.loadTokenCache();
        if (cached && cached.token && cached.baseURL === authManager.baseURL) {
          token = cached.token;
          logger.debug('Using cached authentication token');
        }
      }
      ...
      // If no cached token, authenticate
      if (!token) {
        const { email, password } = await authManager.getCredentials(options);
        token = await authManager.login(email, password);
      }
      
      // Create APIClient instance with authentication
      const api = createAPIClient({ baseURL: options.url || authManager.baseURL });
      api.setAuthToken(token);
      
      logger.debug('Created API client with token', {
