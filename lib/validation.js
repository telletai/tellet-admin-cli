...
function validateEmail(email) {
  ...
  const hasDoubleDots = trimmed.includes('..');
  const startsWithDot = trimmed.startsWith('.') || trimmed.includes('@.');
  
  if (!emailRegex.test(trimmed) || hasDoubleDots || startsWithDot) {
...
function validateDate(dateString, field = 'date') {
  ...
  // Accept ISO date format (YYYY-MM-DD) or similar
  const dateRegex = /^\d{4}-\d{2}-\d{2}/;
  if (!dateRegex.test(trimmed)) {
    throw new ValidationError(
      `Invalid date format for ${field}. Use YYYY-MM-DD format`,
      field,
      dateString
    );
  }
...
function validateFilePath(filePath, options = {}) {
  ...
  const sensitivePatterns = ['/etc/', '\\windows\\', '\\system32\\', '/root/', '.ssh'];
 ...
 * @example
 * const count = validatePositiveInt('42'); // Returns: 42
 * const count2 = validatePositiveInt('abc', 10); // Returns: 10
 * const count3 = validatePositiveInt(-5, 10); // Returns: 10
 */
function validatePositiveInt(value, defaultValue, field = 'value', options = {}) {
  ...
  // Check if value is not a valid integer (has decimals or is not a number)
  if (isNaN(parsed) || parsed < min || parsed > max || (typeof value === 'string' && value.includes('.'))) {
...
function validateOptions(options, schema) {
  ...
  for (const [key, rules] of Object.entries(schema)) {
    ...
    switch (rules.type) {
      ...
      case 'int':
        const intValue = validatePositiveInt(value, rules.default, key, rules);
        if (intValue === null) {
          throw new ValidationError(
            `${key} must be between ${rules.min || 0} and ${rules.max || Infinity}`,
            key,
            value
          );
        }
        validated[key] = intValue;
        break;
      
      case 'enum':
        if (!rules.enum || !Array.isArray(rules.enum)) {
          throw new ValidationError(
            `Invalid enum configuration for ${key}`,
            key,
            value
          );
        }
        validated[key] = validateEnum(value, rules.enum, key);
        break;
      ...
      case 'string':
        ...
        if (rules.enum && !rules.enum.includes(value)) {
          throw new ValidationError(
            `Invalid value for ${key}. Must be one of: ${rules.enum.join(', ')}`,
            key,
            value
          );
        }
