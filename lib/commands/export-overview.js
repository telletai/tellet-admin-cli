...
const { ensureDirectoryExists, generateFilename } = require('../utils/file-utils');
const { createCSVWriter } = require('../utils/stream-utils');
...
const optionsSchema = {
  ...
  status: { type: 'string', enum: ['all', 'in_progress', 'done', 'digested'] },
  fromDate: { type: 'date' },
  toDate: { type: 'date' },
...
async function exportOverviewHandler(options) {
  ...
  try {
    ...
    const project = await api.get(`/analyzer/projects/${projectId}`);
    ...
    const conversations = await api.get('/analyzer/results/conversations', { params });
    ...
    // Create CSV writer
    const csvWriter = createCSVWriter(filepath, headers);
    ...
    for (const conv of conversations) {
      const record = {
        ...
        digest_preview: conv.digest ? conv.digest.substring(0, 100) + '...' : ''
      };
