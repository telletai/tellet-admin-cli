...
async function healthCheckHandler(options) {
  ...
  try {
    ...
    if (validatedOptions.workspace) {
      ...
      try {
        ...
        for (const org of organizations) {
          try {
            const workspaces = await api.get(`/organizations/${org._id}/workspaces`);
            const allWorkspaces = [
    ...
    const projects = await api.get(`/organizations/${workspace.organization_id}/workspaces/${workspace._id}/projects`);
    ...
    for (const project of projects) {
      try {
        const conversations = await api.get(`/analyzer/results/${project._id}/conversations`);
