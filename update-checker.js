...
class GitUpdateChecker {
    ...
    async checkForUpdate(silent = false) {
        ...
        try {
            // For GitHub, check releases API
            const apiUrl = this.repoUrl.replace('https://github.com/', 'https://api.github.com/repos/') + '/releases/latest';
    ...
    displayUpdateNotification(updateInfo) {
        ...
        if (updateInfo.releaseUrl) {
            console.log(chalk.yellow(`   Release notes:   ${updateInfo.releaseUrl}`));
        }
        console.log('');
        console.log(chalk.cyan(`   Update with: npm update -g git+${this.repoUrl}.git`));
    ...
    async performUpdate() {
        ...
        try {
            // Get the Git URL format
            const gitUrl = `git+${this.repoUrl}.git`;
            
            // Try to update globally
            execSync(`npm install -g ${gitUrl}`, { 
                stdio: 'inherit',
                encoding: 'utf8'
            });
            ...
            console.error(chalk.yellow(`   sudo npm install -g git+${this.repoUrl}.git`));
            return false;
        }
    }
